...
function getDayName(date) {
  ...
  return dayWeek[getNumberDay(date)];
}
...
function getNextFriday(date) {
  ...
  const countDay = numberDay < 5 ? 5 - numberDay : 12 - numberDay;
...
function formatDate(date) {
  ...
  return `${currentDate.getUTCMonth() + 1}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()}, ${hours > 12 ? hours - 12 : hours}:${currentDate.getUTCMinutes().toString().padStart(2, 0)}:${currentDate.getUTCSeconds().toString().padStart(2, 0)} ${hours >= 12 ? 'PM' : 'AM'}`;
}
...
function getNextFridayThe13th(date) {
  const currentDate = new Date(date);
  let numberMonth = currentDate.getMonth() - (currentDate.getUTCDate() < 13);
  let date13;
  do {
    numberMonth += 1;
    date13 = new Date(currentDate.getUTCFullYear(), numberMonth, 13);
  } while (getNumberDay(date13) !== 5);
  return date13;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const startPeriod = period.start.split('-');
  let currentDate = new Date(
    startPeriod[2],
    startPeriod[1] - 1,
    startPeriod[0]
  );
  const endPeriod = period.end.split('-');
  const endDate = new Date(endPeriod[2], endPeriod[1] - 1, endPeriod[0]);
  ...
  while (currentDate <= endDate) {
    ...
    if (currentCountWorkDays <= countWorkDays) {
      ...
      currentDate = new Date(
        ...
        currentDate.getUTCDate() + countOffDays
      );
